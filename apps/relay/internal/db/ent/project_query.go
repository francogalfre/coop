...
func (_q *ProjectQuery) QueryInvites() *ProjectInviteQuery {
	query := (&ProjectInviteClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *ProjectQuery) QuerySessions() *AgentSessionQuery {
...
func (_q *ProjectQuery) WithInvites(opts ...func(*ProjectInviteQuery)) *ProjectQuery {
	query := (&ProjectInviteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) WithSessions(opts ...func(*AgentSessionQuery)) *ProjectQuery {
	query := (&AgentSessionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withInvites; query != nil {
		if err := _q.loadInvites(ctx, query, nodes,
			func(n *Project) { n.Edges.Invites = []*ProjectInvite{} },
			func(n *Project, e *ProjectInvite) { n.Edges.Invites = append(n.Edges.Invites, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSessions; query != nil {
		if err := _q.loadSessions(ctx, query, nodes,
			func(n *Project) { n.Edges.Sessions = []*AgentSession{} },
			func(n *Project, e *AgentSession) { n.Edges.Sessions = append(n.Edges.Sessions, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ProjectQuery) loadInvites(ctx context.Context, query *ProjectInviteQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectInvite)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.ProjectInvite(func(s *sql.Selector) {
	...
	for _, n := range neighbors {
		fk := n.project_invites
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_invites" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_invites" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *ProjectQuery) loadSessions(ctx context.Context, query *AgentSessionQuery, nodes []*Project, init func(*Project), assign func(*Project, *AgentSession)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.project_sessions
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_sessions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_sessions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *ProjectQuery) loadAgents(ctx context.Context, query *AgentQuery, nodes []*Project, init func(*Project), assign func(*Project, *Agent)) error {
	...
	for _, n := range neighbors {
		fk := n.project_agents
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_agents" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
