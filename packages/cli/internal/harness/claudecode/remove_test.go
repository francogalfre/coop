...
func TestRemoveSettingsDeletesFileWhenOnlyCoopEntriesExisted(t *testing.T) {
	dir := t.TempDir()
	...
	if err := removeSettings(dir); err != nil {
		t.Fatalf("removeSettings() error = %v", err)
	}

	if _, err := os.Stat(path); !os.IsNotExist(err) {
		t.Fatalf("settings file still exists after removeSettings, err = %v", err)
	}
}

func TestRemoveSettingsPreservesForeignEntriesAndOtherKeys(t *testing.T) {
	dir := t.TempDir()
	...
	if _, err := writeSettings(dir, "http://127.0.0.1:12345"); err != nil {
		t.Fatalf("writeSettings() error = %v", err)
	}
	...
	if _, ok := root["permissions"]; !ok {
		t.Fatal("permissions key was dropped by removeSettings")
	}
	...
	for _, g := range groups {
		for _, e := range g.Hooks {
			if isCoopEntry(e) {
				t.Error("coop entry survived removeSettings")
			}
		}
	}
...
func TestRemoveSettingsNoopWhenFileAbsent(t *testing.T) {
	dir := t.TempDir()

	if err := removeSettings(dir); err != nil {
		t.Fatalf("removeSettings() error = %v, want nil for absent file", err)
	}
}

func TestRemoveSettingsIsInverseOfWriteEventKeys(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, settingsDir, settingsFileName)
